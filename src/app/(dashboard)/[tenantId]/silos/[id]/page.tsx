  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        >
          <ChevronLeft className="w-4 h-4" /> Silos
        </Link>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">{silo.name}</h1>
            <p className="text-muted-foreground text-sm">Capacidade: {capacity.toLocaleString()} {silo.unit}</p>
          </div>
      ...
      <div className="rounded-lg border border-border bg-card p-4 flex flex-wrap items-center gap-6">
        <div>
          <div className="text-[11px] uppercase tracking-widest text-muted-foreground font-bold">Estoque Atual</div>
          <div className="text-lg font-bold text-foreground mt-1">{stock.toLocaleString()} {silo.unit}</div>
        </div>
        <div className="flex-1 min-w-[160px]">
          <div className="text-[11px] uppercase tracking-widest text-muted-foreground font-bold mb-1.5">Ocupação</div>
