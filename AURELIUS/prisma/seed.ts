...
async function main() {
  ...
  const agroProducts = [
    { sku: "DSL-001", name: "Diesel S-10", price: 8500, cost: 7000, currentStock: 5000, minStock: 500 },
    { sku: "FTZ-001", name: "Fertilizante NPK 4-14-8", price: 4500, cost: 3500, currentStock: 10000, minStock: 1000 },
  ];

  for (const prod of agroProducts) {
    await prisma.product.upsert({
      where: {
        tenantId_sku: {
          tenantId: tenant.id,
  ...
  // Create sample fields (Talões)
  const fields = [
    { name: "Talhão Norte", areaHectares: 250.0, soilType: "Argiloso", status: "active" },
    { name: "Talhão Sul", areaHectares: 180.0, soilType: "Misto", status: "active" },
    { name: "Talhão Leste", areaHectares: 320.0, soilType: "Arenoso", status: "active" },
  ];

  for (const f of fields) {
    const existingField = await prisma.field.findFirst({
      where: { tenantId: tenant.id, name: f.name },
    });
    if (!existingField) {
  ...
  // Create sample crop seasons (Safra)
  const seasons = [
    { name: "Safra Soja 2025/2026", crop: "Soja", startDate: new Date("2025-10-01"), endDate: new Date("2026-03-30"), status: "completed" },
    { name: "Safra Milho 2026", crop: "Milho", startDate: new Date("2026-04-01"), endDate: new Date("2026-09-30"), status: "active" },
  ];

  for (const s of seasons) {
    const existingSeason = await prisma.cropSeason.findFirst({
  ...
  const machineries = [
    { name: "Trator JD-450", type: "Tractor", brand: "John Deere", model: "450M", year: 2022, identifier: "JD450-01", currentHours: 1250 },
    { name: "Colheitadeira NH-80", type: "Harvester", brand: "New Holland", model: "TX 8.90", year: 2023, identifier: "NH80-01", currentHours: 620 },
  ];
