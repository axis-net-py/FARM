...
import { createField, updateField } from "@/app/actions/fields";
import type { Field } from "@/app/(dashboard)/[tenantId]/fields/FieldList";

export function FieldSheet({ tenantId, field, onSuccess }: { tenantId: string; field?: Field; onSuccess?: () => void }) {
  ...
  async function handleSubmit() {
    ...
    try {
      if (isEdit && field) {
        await updateField(tenantId, field.id, { name, areaHectares: areaVal, soilType });
      } else {
        await createField(tenantId, { name, areaHectares: areaVal, soilType });
      }
      setOpen(false);
      onSuccess?.();
    } catch (err: any) {
      ...
      <DialogContent className="sm:max-w-[425px] bg-card border border-border p-0 overflow-hidden rounded-lg shadow-lg">
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="p-6 flex flex-col gap-4">
          ...
          <div className="pt-4 flex gap-3 justify-end">
            <button type="button" onClick={() => setOpen(false)} className="px-4 h-[40px] rounded-lg text-[14px] font-semibold text-muted-foreground hover:bg-muted transition-all border border-border">Cancelar</button>
